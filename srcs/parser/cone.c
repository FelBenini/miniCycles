...
void	process_cone(t_scene *scene, char *line)
{
	...
	matched = sscanf(line, "co %f,%f,%f %f,%f,%f %f,%f %f,%f,%f %f %f %f,%f,%f %s %s %s %f",
		&px, &py, &pz,
		&nx, &ny, &nz,
		&radius, &height,
		&r, &g, &b,
	...
	cone = generate_cone(32, 32, radius, height);
	cone.position = vec4_create(px, py, pz, 1.0f);
	cone.direction = vec4_create(nx, ny, nz, 1.0f);
	...
	scene_add_mesh(scene, cone, material_idx);
}
